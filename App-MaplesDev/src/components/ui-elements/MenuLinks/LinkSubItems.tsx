import type { SubLinksBoxProps } from './interfaces.ts';
import { Link } from 'react-router-dom';

export const SubLinksBox = (props: SubLinksBoxProps) => {
    const { subLinks, extraLinks } = props;
    return (
        <div className="absolute top-10 w-fit bg-slate-50 shadow-[rgba(0,_0,_0,_0.24)_0px_0px_40px] shadow-slate-400 hidden group-hover:flex rounded-x1">
            <ul className="p-2 h-fit w-96 flex-col">
                {subLinks.map((subLink) => (
                    <li
                        key={subLink.name}
                        className="h-16 hover:bg-slate-200 rounded-x1 flex items-center justify-start cursor-pointer font-bold"
                    >
